...
CValidacionUsuario.prototype.modificar=function(fullname,username,oldpassword,password,archivoimagen,email,mensajeError){try{var _cm=new ConexionManager();var parametros=new SOAPClientParameters();var _nuevoUsuario=new Usuario();_nuevoUsuario.nombreCompleto=fullname;_nuevoUsuario.nombreUsuario=username;_nuevoUsuario.Password=password;_nuevoUsuario.oldPassword=oldpassword;_nuevoUsuario.email=email;for(var p in _nuevoUsuario){if(_nuevoUsuario[p]==null)
_nuevoUsuario[p]="";}
parametros.add("usuario",JSON.stringify(_nuevoUsuario));var _reg=_cm.conexion("actualizarUsuario",parametros);if(!_reg[0]){$("#lblMensajeError").show("slide",null,"500",null);$("#lblMensajeError").css({'color':'red','font-weight':'bold'});$("#lblMensajeError").text(_reg[1]);}
